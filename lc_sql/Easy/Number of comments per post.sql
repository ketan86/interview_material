...
SELECT s1.post_id as post_id, IFNULL(s2.total_posts, 0) as number_of_comments
FROM (
    -- +---------+------------+
    -- | sub_id  | parent_id  |
    -- +---------+------------+
...
) as s1 LEFT OUTER JOIN (
    -- +--------------+------------+
    -- | total_posts  | parent_id  |
    -- +--------------+------------+
    -- | 3            | 1          |
    -- | 2            | 2          |
    -- | 1            | 7          |
    -- +--------------+------------+
    SELECT COUNT(DISTINCT sub_id) as total_posts, parent_id
    FROM Submissions
