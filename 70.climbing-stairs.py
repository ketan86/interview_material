...
class Solution:
    def climbStairs(self, n):
        ...
        return self._find_ways(n, total_ways, stair_map)

    def _find_ways(self, n, total_ways, stair_map):
        ...
        if n >= 1:
            total_ways = self._find_ways(n - 1, total_ways, stair_map)

        # if we have two or more stairs, one could climb two stairs.
        if n >= 2:
            total_ways = self._find_ways(n - 2, total_ways, stair_map)

        # store total ways one could climb n stairs in the map.
        stair_map[n] = total_ways
...
print(Solution().climbStairsTabulation(4))
# @lc code=end
