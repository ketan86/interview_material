...
def find_single_numbers(nums):
    ...
    #                    XOR of 4 and 6 would be resulted in a number
    #                    where first set bit (1) from right, would be
    ...
    # ***Question***: How do the matching numbers falls into same group so that
    # final num1 and num2 are not affected ?
    # Answer: because we use the `rightmost_set_bit` bit, other numbers in pair
    # would either have that bit set or not set. since both numbers are same,
    # they would fall in one group only.
    # forex,
    #    [1,2,3,2]
    #    XOR of all the numbers -> 2 and second bit is set (10)
    #    [1]         [2,3,2] -> [3]
    #    1 & 2 -> 0
    #    2 & 2 -> 2
    #    3 & 2 -> 2
