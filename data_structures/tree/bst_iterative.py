...
class BinarySearchTree:
    ...
    def in_order_iterative(self):
        ...
        stack = []
        # run the loop until stack is empty or root is not None.
        while stack or root:
            # if root is not None, append to stack and go to left
            if root:
                stack.append(root)
                root = root.left
            # if root is none, pop the root and go to right
            else:
                root = stack.pop()
                print(root.data)
                root = root.right
