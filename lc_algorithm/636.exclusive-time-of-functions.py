...
Share On a single-threaded CPU, we execute a program containing n functions.
...
class Solution:
    def exclusiveTime(self, n: int, logs):
        ...
        # store the total time each function spent.
        result = [0] * n
        stack = []
        last_start_time = 0
        ...
        for l in logs:
            l = l.split(':')
            f, action, time = int(l[0]), l[1], int(l[2])

            if action == 'start':
                start_time = time
                # if action is start and stack is not empty,
                # mark prev function
                if stack:
                    prev_func = stack[-1]
                    # complete the prev task by adding the time diff
                    result[prev_func] += start_time - last_start_time

                # add current task in stack
                stack.append(f)
                # update the last end time to new start time
                last_start_time = start_time
            else:
                end_time = time
                # if end is found, remove the prev function from the
                # stack and update the result for prev func.
                prev_func = stack.pop()
                result[prev_func] += end_time - last_start_time + 1
                # next start time will be end_time + 1
                last_start_time = end_time + 1
