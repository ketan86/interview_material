...
That means both can be accessed in O(1) time. Super fast updates.
...
class LRUCache:
    ...
    def get(self, key):
        if key not in self.cache:
            ...
            # Use last=False to move element to front (orderdict only has `move_to_end` for both operations)
