"""
Given the head of a LinkedList and a number ‘k’, rotate linked list by k
nodes.
...
def rotate(head, k):
    ...
    # find the head using two pointers
    new_start = head
    end = None
    while start:
        end = start
    ...
    # set start to point to the head
    end.next = head
    head = new_start
    prev.next = None
...
#     # once we find the node where first node should be places,
