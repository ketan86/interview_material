...
def square_sorted_tp(arr):
    # define a new array with same number of element of existing array
    new_array = [None] * len(arr)
    # use two pointers
    start = 0
    end = len(arr) - 1

    # index to populate array from right to left
    index = len(arr) - 1
    # loop over until start and end is same.
    while start <= end:
        # find square of two numbers
        s_square = arr[start] * arr[start]
        e_square = arr[end] * arr[end]
        # if square of the start element is greater than end,
        # add start element square into new array and increment
        # start index.
