...
def rotate_clockwise(n, k):
    # left shift number by size-k times to move last k bits to left side
    isolated_bits = n << (BIT_32_INTEGER - k)
    # right shift number by k times to strip off last k bits.
    right_shifted_n = n >> k

    # OR both results to get the final answer
    return isolated_bits | right_shifted_n


def rotate_anticlockwise(n, k):
    # right shift number by size-k times to move last k bits to right side
    isolated_bits = n >> (BIT_32_INTEGER - k)
    # left shift number by k times to move k bits to left.
    right_shifted_n = n << k
