...
structure containing the number of clicks that were recorded on each domain AND
each subdomain under it. For example, a click on "mail.yahoo.com" counts toward
the totals for "mail.yahoo.com", "yahoo.com", and "com". (Subdomains are added
to the left of their parent domain. So "mail" and "mail.yahoo" are not valid
domains. Note that "mobile.sports" appears as a separate domain near the bottom
of the input.)
...
# Leetcode REF : https://leetcode.com/problems/subdomain-visit-count/

"""
Algorithm:
    space : O(n)
    time : m domains x n subdomains per domain
        O(m x n)
    
    1. use map to store the domain and count
        d = {
            'google.com' : 0
            'com' : 0 
        }
    ...
    2. read each input
        - process domain from left to right
            - update/set domain count
            - remove child domain
...
class Solution:
    def subdomainVisits(self, cpdomains):
        """Runtime: 48 ms, faster than 88.99%"""
        result = []
        if not cpdomains:
            return result
