...
class Solution:
    ...
    In an UNSIGNED 32-bit number, the valid values are from 0 to (2^32)-1
        ...
        billion).
        1..2^32 or 1-1..(2^32)-1 = 0..((2^32) - 1) => 4,294,967,296 - 1 => 4,294,967,295
    ...
    def divide(self, dividend, divisor):
        ...
        # both are negative, we will get max unsigned integer
        ...
        for i in range(Solution.INT_MAX_BITS, -1, -1):  # start, stop and steps
            number = divisor << i
            if dividend >= number:
                dividend -= number
                quotient += 1 << i
